...
presentación (Ej 10s de material por paquete), lo que permite simplificar
...
de datos privados sea bastante difícil (Ej requiera esfuerzo) para que el
interesado (Ej atacante) renuncie a la tarea de romper el código criptográfico.
...
2. el draft [[https://dvcs.w3.org/hg/html-media/raw-file/tip/encrypted-media/encrypted-media.html][Encrypted Media Extensions]] (EME)
   ...
   permite enchufar DRMs (Ej programas privativos cerrados) tradicionales
